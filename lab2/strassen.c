...
void *strassen_algo(void *arg) {
	...
	switch (thread_id) {
		...
		default: 
			printf("There's no option for %ld\n", thread_id);
	}
...
int main(int argc, char *argv[])
{
	...
	// puts("Se initializeaza bariera..\n");
	pthread_barrier_init(&barrier, NULL, NUM_THREADS);
	if (r) {
		printf("Cannot init barrier!\n");
	}
	
	// puts ("Se creaza threadurile..\n");
	for (long id = 0; id < NUM_THREADS; id++) {
        r = pthread_create(&threads[id], NULL, strassen_algo, (void *)id);

		// printf("S-a creat thread-ul %ld.\n", id);
        if (r) {
	...
	// puts ("Se distruge bariera..\n");
	r = pthread_barrier_destroy(&barrier);
	if (r) {
		printf("Cannot destroy barrier.\n");
	}

	// puts ("Se face sincronizarea thread-urilor..\n");
